...
define([{{#each log.nonJSONEntries}}
    "text!{{{request.url}}}"{{#unless last}},{{/unless}}{{/each}}
], function () {
    ...
    var staticFiles = [{{#each log.nonJSONEntries}}
            "{{{request.url}}}"{{#unless last}},{{/unless}}{{/each}}
        ],
        deps = arguments;
